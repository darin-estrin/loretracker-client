...
class StartCampaign extends Component {
  ...
  render() {
    ...
    return(
      <div>
        <form className='campaign-start' onSubmit={handleSubmit(this.handleFormSubmit)}>
...
function validate(formProps) {
  ...
  if (!validator.validate(email1) && email1 !== '' && email1 !== undefined) {
    errors.email1 = 'Please enter a valid player email address';
  }

  if (!validator.validate(email2) && email2 !== '' && email2 !== undefined) {
    errors.email2 = 'Please enter a valid player email address';
  }

  if (!validator.validate(email3) && email3 !== '' && email3 !== undefined) {
    errors.email3 = 'Please enter a valid player email address';
  }

  if (!validator.validate(email4) && email4 !== '' && email4 !== undefined) {
    errors.email4 = 'Please enter a valid player email address';
  }

  if (!validator.validate(email5) && email5 !== '' && email5 !== undefined) {
    errors.email5 = 'Please enter a valid player email address';
  }

  if (!validator.validate(email6) && email6 !== '' && email6 !== undefined) {
